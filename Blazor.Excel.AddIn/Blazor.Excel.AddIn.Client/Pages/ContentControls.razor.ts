...
export async function setupDocument() {
  await Excel.run(async (context) => {
    ...
    context.document.body.insertParagraph(
      "Co-locating Index.razor.js Demo",
      "Start"
    );
    context.document.body.insertParagraph(
      "Inserting another paragraph. ",
      "Start"
    );
    ...
    context.document.body.paragraphs
      .getLast()
      .insertText(
        "With Excel add-ins, you can use familiar web technologies such as HTML, CSS, and JavaScript to build a solution that can run in Excel across multiple platforms, including on the web, Windows, Mac, and iPad. Learn how to build, test, debug, and publish Excel add-ins.",
        "Replace"
      );
  });
}
...
export async function tagContentControls() {
  ...
  await Excel.run(async (context) => {
    ...
    await context.sync();
    console.log(
      "Content controls tagged and handled: " + contentcontrolsTagged
    );
  });
}
...
export async function modifyContentControls() {
  ...
  await Excel.run(async (context) => {
    ...
    for (let i = 0; i < evenContentControls.items.length; i++) {
      ...
      evenContentControls.items[i].insertHtml(
        "This is an <strong>even</strong> content control",
        "End"
      );
    }
    ...
    for (let j = 0; j < oddContentControls.items.length; j++) {
      ...
      oddContentControls.items[j].insertParagraph(
        "This is an odd content control",
        "End"
      );
    }
...
export async function registerEvents() {
  ...
  await Excel.run(async (context) => {
    ...
    if (contentcontrols.items.length === 0) {
      console.log(
        "There aren't any content controls in this document so can't register event handlers."
      );
    } else {
      ...
      for (let i = 0; i < contentcontrols.items.length; i++) {
        eventContexts[i * 2] = contentcontrols.items[i].onDeleted.add(
          handleContentControlDeleted
        );
        console.log("Added onDeleted handler.");
        eventContexts[i * 2 + 1] = contentcontrols.items[
          i
...
export async function deleteContentControl() {
  await Excel.run(async (context) => {
    ...
    if (contentControls.items.length === 0) {
      console.log(
        "There are no content controls tagged 'even' in this document."
      );
    } else {
...
export async function deregisterEvents() {
  await Excel.run(async (context) => {
    for (let i = 0; i < eventContexts.length; i++) {
      await Excel.run(eventContexts[i].context, async (context) => {
